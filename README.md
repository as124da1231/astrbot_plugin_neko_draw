...
- **APNG 设置**：控制 `apng` 指令成品的外显金句、合并转发、@ 触发者、默认首帧、动画参数和发送后清理。
...
| 设置 | 默认值 |
| --- | --- |
| APNG 制作时发送提示 | 开启，`🐱 猫娘正在制作 APNG，请稍候...` |
| APNG 外显金句 | 关闭 |
| APNG 合并转发 | 关闭 |
| 发送 APNG 时 @ 触发者 | 关闭 |
| APNG 默认首帧 | 插件内置猫娘图片 |
...
- 支持在图片编辑历史中查看提交给模型的原图
- APNG 作品与 AI 生成历史分开管理
- 提供三种 WebUI 主题及响应式布局
...
启用生图首帧包装后，临时 APNG 只负责本次消息发送，发送完成即自动清理；生成历史只保留、查看和下载模型返回的原始生成图。`apng` 指令制作的动画属于独立作品，不受此清理逻辑影响。

插件的分层与配置生效流程见 [ARCHITECTURE.md](ARCHITECTURE.md)。

## 模型提供商与默认模型

“模型提供商”是与“生成历史”并列的独立页面，不占用插件配置区域。页面按照 AstrBot 的使用方式采用左右分栏：左侧选择或新增提供商，右侧直接编辑连接、高级配置和模型列表。插件提供商只管理插件自己的 URL、Key 与图像协议，不会混入 AstrBot 提供商设置。以硅基流动为例，填写 `https://api.siliconflow.cn/v1` 和 API Key，然后点击“获取模型列表”。连接成功后会显示接口返回的模型；搜索框同时过滤已配置模型和待添加模型。服务商返回的模型只能从列表中选择，只有点击“自定义模型”时才允许手工填写模型 ID。选择模型后，可以继续设置用途（文生图或图片编辑）、显示名称、参考图字段、回退优先级和请求参数。

OpenAI 兼容模型会自动在根地址后使用 `images/generations` 端点，例如最终请求为 `https://api.siliconflow.cn/v1/images/generations`。WaveSpeed 与 RunningHub 则按各自协议把模型路径拼入请求地址。模型自定义参数能力保持不变。

“默认模型选择”与提供商配置分离，包含“默认文生图模型”和“默认图片编辑模型”两个下拉框。下拉框同时列出猫娘画图中已启用的对应用途模型，以及 AstrBot 当前已启用的模型。保存后模型客户端立即刷新，不需要再重启插件。
...
进入插件 WebUI 的独立“模型提供商”页面添加和管理 API 凭据。可以添加多个相同协议的提供商，例如 OpenAI 官方接口、兼容中转服务和另一组备用账号。连接测试会读取模型列表，不会提交图片生成任务。

| 字段 | 说明 |
| --- | --- |
| 提供商 ID | 插件内唯一的连接名称 |
| 图像接口协议 | 选择 WaveSpeed、RunningHub 或 OpenAI 兼容，决定调用路径和数据格式 |
| API Key | 该提供商使用的访问密钥 |
| API Base URL | 该提供商的接口根地址 |
...
| 名称 | 类型 | 默认 Base URL |
| --- | --- | --- |
| WaveSpeed | WaveSpeed | `https://api.wavespeed.ai/api/v3` |
...
每个模型代表一套可以独立调用的图片生成配置，并直接归属于当前提供商。
...
图片编辑任务会将实际提交给模型的原图保存到插件数据目录，并在记录详情的“编辑原图”区域展示。删除该条历史或清空生成历史时，对应的原图副本也会删除。

## APNG 文件与清理

`apng` 指令制作的动画默认保存在服务器的插件数据目录，并显示在独立的“APNG 作品”页面。该页面支持预览、下载、逐条删除和全部清空；删除记录时会同时删除对应 APNG 文件，但不会影响已经发送到 QQ 的消息。

如果开启“APNG 发送后立即清理”，插件会等发送完成后删除服务器文件，并且不写入 APNG 作品记录。制作过程中下载的 QQ 临时原图无论该开关是否开启都会在合成结束后清理。
