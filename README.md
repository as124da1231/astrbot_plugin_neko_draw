...
插件会根据 URL 自动识别 WaveSpeed、RunningHub 或 OpenAI 兼容协议；无法识别的地址默认按 OpenAI 兼容处理。OpenAI 兼容模型会自动使用 `images/generations` 端点；WaveSpeed 会把模型 ID 拼接到请求路径，因此不提供模型搜索，需通过“自定义模型”填写；RunningHub 使用自己的任务提交和轮询流程。模型自定义参数能力保持不变。
...
每个模型代表一套可以独立调用的图片生成配置，并直接归属于当前提供商。首次添加主流模型时，WebUI 会根据模型 ID 和提供商地址自动填入推荐请求参数；已保存的人工设置不会被后续识别结果覆盖，需要重置时可主动点击“恢复推荐参数”。所有推荐参数仍可自由增加、删除或修改。
